...
function EditHabitForm({ habit, onHabitUpdated, onClose }) {
  const [name, setName] = useState(habit.name);
  const [description, setDescription] = useState(habit.description || "");
  const [frequency, setFrequency] = useState(habit.frequency);
  const [timesPerDay, setTimesPerDay] = useState(habit.times_per_day);
  const [color, setColor] = useState(habit.color);
  const [illustrations, setIllustrations] = useState([]);
  const [selectedIllustration, setSelectedIllustration] = useState(habit.illustration);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name) return;
    await axios.put(`http://localhost:3001/api/habits/${habit.id}`, {
      name,
      description,
      frequency,
